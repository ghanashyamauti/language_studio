...
def login(payload: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if role == "student":
        ...
        if row and verify_password(password, row.password_hash):
            ...
            extra = {"class_": row.class_, "semester": row.semester, "roll_no": row.roll_no, "prn": row.prn, "must_change_password": row.must_change_password}
            actor_id, actor_name = row.student_id, row.name

    elif role == "teacher":
        norm = username.replace(" ", "")
        row = next((t for t in db.query(Teacher).all() if (t.phone or "").replace(" ", "") == norm), None)
        if row and verify_password(password, row.password_hash):
            user = row
            token_data = {"sub": str(row.teacher_id), "role": "teacher", "name": row.name}
            extra = {"must_change_password": row.must_change_password}
            actor_id, actor_name = row.teacher_id, row.name

    elif role == "hod":
        norm = username.replace(" ", "")
        row = next((h for h in db.query(HOD).all() if (h.phone or "").replace(" ", "") == norm), None)
        if not row:
        ...
        if row and verify_password(password, row.password_hash):
            ...
            extra = {"phone": row.phone, "department": row.department, "managed_classes": managed}
