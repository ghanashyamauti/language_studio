...
export default function StudentSettings() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/student/change-password', form);
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="card space-y-6">
        ...
        <div className="flex flex-col items-center sm:flex-row gap-5 pb-5 border-b border-gray-100 animate-fadeIn">
          <div className="relative group">
            {user?.extra?.profile_photo ? (
              <img 
                ...
                alt="Profile" 
                className="w-24 h-24 rounded-full object-cover border-2 border-rose-500/20 shadow-md animate-scaleUp" 
              />
            ) : (
              <div className="w-24 h-24 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center text-3xl font-bold border-2 border-rose-500/20 shadow-inner select-none animate-scaleUp">
          ...
          <div>
            <h3 className="font-bold text-slate-800 text-base">{user?.name || 'Student Name'}</h3>
            <p className="text-xs text-slate-500 capitalize">Student Portal ({user?.extra?.class_})</p>
