...
export default function TeacherFaceAttendance() {
  ...
  const checkStatus = async () => {
    try {
      // Registration status
      const regRes = await api.get('/teacher/face-status');
      ...
      // Attendance history
      const historyRes = await api.get('/teacher/my-attendance?limit=10');
  ...
  const handleRegister = async () => {
    ...
    setMarking(true);
    try {
      await api.post('/teacher/face-register', { face_descriptor: descriptor });
  ...
  const handleMarkAttendance = async () => {
    ...
    setMarking(true);
    try {
      const res = await api.post('/teacher/face-attendance', { face_descriptor: descriptor });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-jspm-navy">Face Attendance</h1>
        <p className="text-gray-500 text-sm">Verify your face via webcam to mark daily attendance</p>
      </div>
